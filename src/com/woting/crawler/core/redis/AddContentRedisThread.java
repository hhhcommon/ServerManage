...
public class AddContentRedisThread extends Thread {
	...
	private Map<String, Object> makeSeqMediaAssetInfo(String smaId) {
		...
		try {
			...
			String sql = "SELECT id,smaTitle,smaPublisher,descn,smaImg,cTime FROM wt_SeqMediaAsset where id = '"+smaId+"'";
			try {
			...
			sql = "SELECT kw.id,kw.kwName FROM wt_Kw_Res kwsf"
					+ " LEFT JOIN wt_KeyWord kw ON kw.id = kwsf.kwId"
					+ " where kwsf.resId = '"+smaId+"' and kwsf.resTableName = 'wt_SeqMediaAsset'";
			try {
	...
	private List<Map<String, Object>> getMediaAssetInfos(List<String> maIds) {
		...
		try {
			if (maIds!=null && maIds.size()>0) {
				...
				ids = ids.substring(3);
				ids = "("+ids+")";
				String sql = "SELECT ma.id,ma.maTitle,ma.maPublisher,ma.descn,ma.maImg,ma.cTime,mas.playURI FROM wt_MediaAsset ma"
				...
				try {
					...
					while (rs != null && rs.next()) {
						...
						oneDate.put("ContentId", rs.getString("id"));
						oneDate.put("ContentPublisher", rs.getString("maPublisher"));
						oneDate.put("MediaType", "AUDIO");
						oneDate.put("ContentDesc", rs.getString("descn"));
						oneDate.put("ContentImg", rs.getString("maImg"));
						oneDate.put("ContentTime", rs.getTimestamp("cTime").getTime());
				...
				sql = "SELECT kw.id,kw.kwName,kwsf.resId FROM wt_Kw_Res kwsf"
						+ " LEFT JOIN wt_KeyWord kw ON kw.id = kwsf.kwId"
						+ " where "+ids.replace("id", "kwsf.resId")+" and kwsf.resTableName = 'wt_MediaAsset'";
				try {
					ps = conn.prepareStatement(sql);
					rs = ps.executeQuery();
					while (rs != null && rs.next()) {
						...
						for (Map<String, Object> map : retLs) {
							if (map.get("ContentId").equals(rs.getString("resId"))) {
								if (map.containsKey("KeyWords")) {
									List<Map<String, Object>> kwmapls = (List<Map<String, Object>>) map.get("KeyWords");
									if (kwmapls!=null) {
										kwmapls.add(kwmap);
									} else {
										kwmapls = new ArrayList<>();
										kwmapls.add(kwmap);
										map.put("KeyWords", kwmapls);
									}
								} else {
									List<Map<String, Object>> kwmapls = new ArrayList<>();
									kwmapls.add(kwmap);
									map.put("KeyWords", kwmapls);
								}
							}
