...
public class CrawlerSrcRecord {
	...
	public void reloadCrawlerInfo(){
		...
		while (true) {
			for (int i = 1; i <= num; i++) {
				if (RedisUtils.isOrNoCrawlerFinish(rs,i+"")) {
					...
					if(als!=null&&als.size()>0) {
						...
						for (AlbumPo al : als) { //快照格式 albumName+albumPublisher : albumId
							RedisUtils.addCrawlerSrcRecord(rs, al.getAlbumName()+al.getAlbumPublisher(), al.getId());
						}
					}
						...
						for (int j = 0; j <= n/1000; j++) {
							...
							if(aus!=null&&aus.size()>0) {
								for (AudioPo au : aus) { //快照格式 audioURL : audioId
									RedisUtils.addCrawlerSrcRecord(rs, au.getAudioURL(), au.getId());
								}
							}
