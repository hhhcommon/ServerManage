...
public class XMLYCrawler {
	...
	public XMLYCrawler() {
		String str = FileUtils.readFile("/opt/CrawlerCS/XMLYREF.txt");
...
//			catemap.putAll(catemap2);
			...
			FileUtils.writeFile(JsonUtils.objToJson(map), "D:\\XMLYREF.txt");
			System.out.println(newls.size());
			int audios = 0;
			List<Integer> iList = new ArrayList<>();
			int audiothreads = newls.size()/2000+1;
			fixedThreadPool = Executors.newFixedThreadPool(audiothreads);
			for (int i = 1; i <= audiothreads; i++) {
				int fonum = i;
				fixedThreadPool.execute(new Runnable() {
					public void run() {
						List<String> ls2 = newls.subList((fonum-1)*2000, fonum*2000>newls.size()?newls.size():fonum*2000);
			...
			FileUtils.writeFile(JsonUtils.objToJson(map), "/opt/CrawlerCS/XMLYREF.txt");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
		...
		try {
			...
			if (alm!=null) {
				...
				if (toInsert) {
				    ...
				    xmlyEtl1Process.insertNewAlbum(alm,map);
				}
